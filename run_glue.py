...
class ModelArguments:
    ...
    mnli_model_name_or_path: str = field(
        default=None, metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"}
    )
...
def main():
    ...
    mnli_config.aug_cutoff_ratio = training_args.aug_cutoff_ratio

    if  model_args.mnli_model_name_or_path is not None:
        mnli_model = AutoModelForSequenceClassification.from_pretrained(
            model_args.mnli_model_name_or_path,
    ...
    config.aug_cutoff_ratio = training_args.aug_cutoff_ratio
