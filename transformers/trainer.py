...
class Trainer:
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.do_aug:
                    ...
                    elif self.args.aug_type == 'hidden_span_cutoff':
                        step_loss = self._training_step_with_hidden_span_cutoff(model, inputs, optimizer)
                    elif self.args.aug_type == 'attn_span_cutoff':
                        step_loss = self._training_step_with_attn_span_cutoff(model, inputs, optimizer, self.args.aug_type)
                    elif self.args.aug_type == 'reverse_attn_span_cutoff':
                        step_loss = self._training_step_with_attn_span_cutoff(model, inputs, optimizer, self.args.aug_type)
                    elif self.args.aug_type == 'hidden_dim_cutoff':
                        step_loss = self._training_step_with_hidden_dim_cutoff(model, inputs, optimizer, self.args.aug_type)
                    elif self.args.aug_type == 'sst2_dim_cutoff':
                        step_loss = self._training_step_with_hidden_dim_cutoff(model, inputs, optimizer, self.args.aug_type)
                    elif self.args.aug_type == 'sst2_dim_cutoff_reverse':
                        step_loss = self._training_step_with_hidden_dim_cutoff(model, inputs, optimizer, self.args.aug_type)
                    elif self.args.aug_type == 'qnli_dim_cutoff':
                        step_loss = self._training_step_with_hidden_dim_cutoff(model, inputs, optimizer, self.args.aug_type)
                    elif self.args.aug_type == 'qnli_dim_cutoff_reverse':
                        step_loss = self._training_step_with_hidden_dim_cutoff(model, inputs, optimizer, self.args.aug_type)
                    else:
                ...
                if (self.args.max_steps > 0 and self.global_step > self.args.max_steps) or (self.args.early_stop > 0 and self.patience > self.args.early_stop):
    ...
    def _training_step_with_hidden_span_cutoff(
            self, model: nn.Module, inputs: Dict[str, torch.Tensor], optimizer: torch.optim.Optimizer
    ) -> float:
        model.train()
        for k, v in inputs.items():
        ...
        inputs['hidden_cutoff'] = 'span_cutoff'
        cutoff_outputs = model(**inputs)
        #print('????????')
        #print(cutoff_outputs[0])
    ...
    def _training_step_with_attn_span_cutoff(
            self, model: nn.Module, inputs: Dict[str, torch.Tensor], optimizer: torch.optim.Optimizer, aug_type = None
    ) -> float:
        model.train()
        for k, v in inputs.items():
            inputs[k] = v.to(self.args.device)
    ...
    def _training_step_with_hidden_dim_cutoff(
            self, model: nn.Module, inputs: Dict[str, torch.Tensor], optimizer: torch.optim.Optimizer, aug_type = None,
    ) -> float:
        model.train()
        for k, v in inputs.items():
        ...
        if aug_type == None:
            inputs['hidden_cutoff'] = 'dim_cutoff'
        else:
            inputs['hidden_cutoff'] = aug_type
        cutoff_outputs = model(**inputs)
        #print('????????')
